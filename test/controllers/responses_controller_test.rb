...
class ResponsesControllerTest < ActionController::TestCase
  ...
  describe '#new' do
    it 'creates multiple unsaved response objects from questions for the view' do
      ...
      feedback.data.each do |data|
        num_responses += data[:survey_responses].count
      end
  ...
  describe '#index' do
    it 'should show all responses' do
      ...
      question = create(:question, :text, :required, survey_id: survey.id)
      response1 = create(:response, question_id: question.id, value: 'value')
